...
public:
     ...
     * @param maxVoltage the maximum voltage as expressed as a 2 byte unsigned integer.
     *                    defaults to the max operating voltage for the DEV1 pack
...
private:
    ...
    /**
     * Sends the requested CAN Message.  To be used for Data request messages.  Not for state control commands
     * Returns a pointer to the response message from the
     * SIM100 device.
     * Waits for RESPONSE_TIMEOUT before returning
     * @param requestType The type of message to request
     * @param responseMessage The response message corresponding to the data request
     * @return 0 if successful
     */
    int sendDataRequestMessage(RequestMux requestType, IO::CANMessage &responseMessage);

    /**
     * Send any CAN message to the SIM100 board.  Constructs to CANMessage and fills the proper response
